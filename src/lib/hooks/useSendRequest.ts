...
interface UsePostRequestProps<T, R> {
  ...
  verificationHash?: { name: string; getValue: (data: R) => string };
}
...
const useSendRequest = <T, R>({
  ...
  const { mutate, isPending, isSuccess } = useMutation({
    ...
    onSuccess: data => {
      ...
      if (verificationHash) {
        const verificationHashValue = verificationHash.getValue(data);
        if (verificationHashValue) {
          sessionStorage.setItem(verificationHash.name, verificationHashValue);
        }
      }
      if (successToast)
